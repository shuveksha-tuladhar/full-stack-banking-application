...
app.post('/login', async (request, response) => {
    ...
    if (userInfo.length > 0) {
        ...
        const accessToken = jwt.sign({ username: user.email, role: user.role }, ACCESS_TOKEN_SECRET, { expiresIn: '20m' });
        const refreshToken = jwt.sign({ username: user.email, role: user.role }, REFRESH_TOKEN_SECRET);
